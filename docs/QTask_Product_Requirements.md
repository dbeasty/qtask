...
- Sharer / collaborator list: every project (and optionally every individual task) has a list of collaborators with defined roles (owner, manager, editor, executor, viewer). Manager can create sub-projects and edit structure but not delete or manage members. Access is evaluated per project; nesting does not automatically grant access to ancestors or descendants.
