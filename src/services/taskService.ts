...
import { generateObjectId } from '../data/kdb/ids.js';
import type { CommentDoc, TaskDoc } from '../data/documents.js';
...
async function saveTaskWithVersionRetry<T extends TaskDoc>(
  taskId: string,
  task: T,
  ...
  for (let attempt = 0; attempt < VERSION_SAVE_MAX_ATTEMPTS; attempt++) {
    try {
      await taskDocs().replaceOne({ _id: current._id }, current);
      ...
      const reloaded = await taskDocs().findById(taskId);
...
function finalizeTaskProgress(task: {
  ...
  const withPercent = applyPercentComplete(
    task as unknown as Parameters<typeof applyPercentComplete>[0]
  );
...
export class TaskService {
  ...
  ): Promise<{ task: TaskDoc; role: ProjectRole }> {
  ...
  assertSubtaskPathExists(
    ...
    const found = this.findSubtaskByPath(
      (task.subtasks ?? []) as Array<Record<string, unknown>>,
      subtaskPath
    );
    if (!found) {
  ...
  private async loadAccessibleTaskWithRole(
    ...
    const task = await taskDocs().findById(taskId);
    if (!task) return null;
    if (task.staging) {
  ...
  async createTask(
    ...
    const minTask = await taskDocs().findOne({
      $or: [{ projectIds: primaryProjectId }, { projectId: primaryProjectId }],
      staging: { $exists: false },
    }, { sort: { sortOrder: 1 }, select: 'sortOrder' });
    draft.sortOrder = minTask ? ((minTask.sortOrder as number | undefined) ?? 0) - 1 : 0;

    const withPercent = applyPercentComplete(draft as unknown as Parameters<typeof applyPercentComplete>[0]);
    draft.status = withPercent.status;
    draft.percentComplete = withPercent.percentComplete;
    draft.subtasks = withPercent.subtasks;

    // The store mints the id and the subdocument ids, so the document only exists
    // once it is created — there is no unsaved-document phase to reason about.
    const taskDoc = await taskDocs().create(draft);
    if (!staging) {
  ...
  async getTask(userId: string, taskId: string, stagingConversationId?: string) {
    ...
    const withPercent = applyPercentComplete(task as unknown as Parameters<typeof applyPercentComplete>[0]);
  ...
  async listTasks(
    ...
    if (filters.query) {
      ...
      const fallbackTasks = await taskDocs().find({
        $and: [
          query,
    ...
    if (pagination) {
      const [tasks, total] = await Promise.all([
        taskDocs().find(query, { sort: TaskService.LIST_SORT, limit: pagination.limit, skip: pagination.offset }),
        taskDocs().countDocuments(query),
      ]);
      return {
    ...
    const tasks = await taskDocs().find(query, { sort: TaskService.LIST_SORT });

    return tasks.map((task) =>
      serializeTask(applyPercentComplete(task as unknown as Parameters<typeof applyPercentComplete>[0]) as unknown as Record<string, unknown>)
    );
  }
  ...
  async deleteTask(userId: string, taskId: string, options: { keepChildren?: boolean } = {}) {
    ...
    if (!keepChildren) {
      await taskDocs().deleteOne({ _id: task._id });
      await commentDocs().deleteMany({ taskId });
      await logActivity({
        taskId,
        userId,
        action: 'task.deleted',
    ...
    const children = (task.subtasks ?? []) as Array<Record<string, unknown>>;

    const promotedDrafts = children.map((child, index) => {
      ...
      const withPercent = applyPercentComplete(
        doc as unknown as Parameters<typeof applyPercentComplete>[0]
      );
      doc.status = withPercent.status;
      doc.percentComplete = withPercent.percentComplete;
      doc.subtasks = withPercent.subtasks;
      return doc;
    });

    // Shift later siblings so promoted children keep relative order after the parent slot.
    if (promotedDrafts.length > 1) {
      await taskDocs().updateMany(
        {
          $or: [{ projectIds: projectId }, { projectId }],
          _id: { $ne: task._id },
          sortOrder: { $gt: parentSortOrder },
        },
        { $inc: { sortOrder: promotedDrafts.length - 1 } }
      );
    }

    const promotedDocs = await Promise.all(promotedDrafts.map((doc) => taskDocs().create(doc)));
    await taskDocs().deleteOne({ _id: task._id });
    await commentDocs().deleteMany({ taskId });

    const promotedTasks = promotedDocs.map((doc) => serializeTask(doc));
  ...
  async addLink(userId: string, taskId: string, linkedTaskId: string, type: TaskLinkType) {
    ...
    if (!task.links.some((l) => l.taskId === linkedTaskId && l.type === type)) {
      ...
      await taskDocs().replaceOne({ _id: task._id }, task);
    }
  ...
  async getWorkload(userId: string, assigneeId?: string) {
    ...
    const tasks = await taskDocs().find(query);
    return tasks
      .map((task) => {
        const withPercent = applyPercentComplete(task as unknown as Parameters<typeof applyPercentComplete>[0]);
        return {
  ...
  async addSubtask(
    ...
    return withTaskSaveLock(taskId, async () => {
      ...
      (newSubtask as { _id: string })._id = generateObjectId();
  ...
  async promoteSubtaskToTask(userId: string, taskId: string, subtaskPath: string[]) {
    ...
    return withTaskSaveLock(taskId, async () => {
      ...
      const promotedDraft: Record<string, unknown> = {
        userId,
        projectId,
        projectIds: [...projectIds],
      ...
      promotedDraft.sortOrder = minTask ? ((minTask.sortOrder as number | undefined) ?? 0) - 1 : 0;

      const withPercent = applyPercentComplete(
        promotedDraft as unknown as Parameters<typeof applyPercentComplete>[0]
      );
      promotedDraft.status = withPercent.status;
      promotedDraft.percentComplete = withPercent.percentComplete;
      promotedDraft.subtasks = withPercent.subtasks;

      const promotedDoc = await taskDocs().create(promotedDraft);
      ...
      return {
        ...
        promotedTask: serializeTask(promotedDoc),
      };
    });
  }
  ...
  async attachTaskAsSubtask(
    ...
    return withTaskSaveLock(targetTaskId, async () => {
      ...
      const sourceObj = sourceTask as Record<string, unknown>;
      const newSubtask = this.taskDocToSubtaskNode(sourceObj);
      const subtaskId = String((newSubtask as { _id: string })._id);
      ...
      await taskDocs().replaceOne({ _id: targetTask._id }, targetTask);
      await taskDocs().deleteOne({ _id: sourceTaskId });
      await commentDocs().deleteMany({ taskId: sourceTaskId });
      ...
      return {
        targetTask: serializeTask(targetTask),
  ...
  async reorderProjectTask(userId: string, projectId: string, taskId: string, index: number) {
    ...
    await Promise.all(
      orderedIds.map((id, sortOrder) => taskDocs().updateOne({ _id: id }, { $set: { sortOrder } }))
    );
  ...
  async duplicateTask(userId: string, taskId: string, projectId: string) {
    ...
    draftDoc.sortOrder = minTask ? ((minTask.sortOrder as number | undefined) ?? 0) - 1 : 0;

    const withPercent = applyPercentComplete(
      draftDoc as unknown as Parameters<typeof applyPercentComplete>[0]
    );
    draftDoc.status = withPercent.status;
    draftDoc.percentComplete = withPercent.percentComplete;
    draftDoc.subtasks = withPercent.subtasks;

    const doc = await taskDocs().create(draftDoc);
  ...
  private getParentArray(
    task: { subtasks: Array<Record<string, unknown>> },
    parentPath: string[]
  ...
  private findSubtaskByPath(
    subtasks: Array<Record<string, unknown>>,
    path: string[]
  ): { subtasks: Array<Record<string, unknown>> } | null {
    ...
    return node as unknown as { subtasks: Array<Record<string, unknown>> } | null;
  }
}
